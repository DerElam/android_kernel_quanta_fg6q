...
{ 
        ...
        send[3] = command & 0xff; 
		dev_dbg(es305->dev, "es305_send_cmd (0x%x)\n",command); 
...
{ 
		...
		dev_dbg(es305->dev, "es305_load_firmware size(%d) ",size); 
        while (size > 0) { 
			i++;
		...
		dev_dbg(es305->dev, ",sent segment %d \n",i); 
...
{ 
        ...
        while (retry--) { 
                 ...
                 */ 
				dev_dbg(es305->dev, "Send boot command(0x%04x 0x%04x)",boot[0],boot[1]); 
				...
				dev_dbg(es305->dev, "get ack command(%x)",ack); 
...
{ 
        ...
        msleep(20); 
		if(pdata->clk_enable!=NULL)
	        pdata->clk_enable(false); 
		if(pdata->gpio_wakeup!=-1)
...
{ 
		...
		dev_dbg(es305->dev, "es305_firmware_ready"); 
        if (!fw) { 
...
{ 
        ...
        es305 = kzalloc(sizeof(struct es305_data), GFP_KERNEL); 
        if ( NULL == es305) { 
                ret = -ENOMEM; 
                goto err_kzalloc; 
        } 
		...
		dev_info(es305->dev, "es305_probe\n");
        ...
        if ((pdata->passthrough_src < 0) || (pdata->passthrough_src > 4) || 
                ...
                ret = -EINVAL; 
                goto err_pdata; 
        } else if ((pdata->passthrough_src != 0) && (pdata->passthrough_dst != 0) && (pdata->passthrough_type != 0)) { 
        		if(pdata->passthrough_type)
	                es305->passthrough = ((pdata->passthrough_src + 0xb) << 4) | 
	                                        ((pdata->passthrough_dst - 1) << 2)|(pdata->passthrough_src-1);
				else
...
{ 
        ...
        if(es305->pdata->gpio_wakeup!=-1)
	        gpio_free(es305->pdata->gpio_wakeup); 
		if(es305->pdata->gpio_reset!=-1)
